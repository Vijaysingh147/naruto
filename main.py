...
bot = Client("bot",
             bot_token= "6317922791:AAHC754ghPtddTRqRcn83EC0NmxA4VfHP0Q",
             api_id= 28362163,
             api_hash= "840be5d60b3594d493a7d96ad6283834"
)
...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1002019491661, x)
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:
                cc = f'[ 🎬 ] **Vid ID : **{str(count).zfill(3)}\n**Video Title :** {name1}({res}) 『 𝗛𝗘𝗠𝗨 』 𝐓𝐄𝐋𝐄𝐆𝐑𝐀𝐌 𝐋𝐈𝐍𝐊 https://t.me/+iA3pT6UPONVlYzg1 𝗛𝗘𝗠𝗨.mp4\n**Batch Name :** {b_name}\n\n**Extracted By ➤** {CR}'
                cc1 = f'[ 📕 ] **Pdf ID : **{str(count).zfill(3)}\n**File Title :** {name1}**{CR}**.pdf\n\n**Batch Name :**{b_name}\n\n**Downloaded By ➤** {CR}'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                        await copy.copy(chat_id = -1002019491661)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
